...
```
                              ...
                              +--------+-------+
                                       ...
                                       |
+---------+    +---------+    +--------v-------+    +-----------+
| Promtail|--->|  Loki   |    | Prometheus     |    | Tempo     |
| /var/log|    | 3100    |    | (remote_write) |    | 3200/4317 |
+----+----+    +---------+    +--------+-------+    +-----------+
     |                                   |                  |
     |                                   |                  |
     +-----------------------------------+                  |
                 Docker network lgtm-net                    |
                                                           +--v--+
                                                           |MinIO |
                                                           |9000  |
                                                           +------+
...
- 결과: MinIO가 헬스체크를 통과한 이후에 종속 서비스가 시작되므로 버킷 생성 및 S3 백엔드 연결 안정성이 확보되었습니다.
